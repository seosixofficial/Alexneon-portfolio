...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function initPhysicsPlayground() {
        ...
        // Function to drop items on mouse move (throttled)
        const neonColor = '#ccff00';
        let lastDropTime = 0;
        
        container.addEventListener('mousemove', (e) => {
            const now = Date.now();
            // Throttle to drop at most 1 item every 150ms
            if (now - lastDropTime < 150) return;
            
            // Limit total bodies to prevent performance crash
            if (world.bodies.length > 150) {
                // remove the oldest dropped body (index 0 is bottom wall, 1 left, 2 right)
            ...
            if (type === 'circle') {
                ...
                body = Bodies.rectangle(x, y, 30 + Math.random() * 30, 15 + Math.random() * 15, commonOptions);
            } else {
                body = Bodies.polygon(x, y, Math.floor(Math.random() * 3) + 3, 15 + Math.random() * 15, commonOptions);
            }
